...
<script>
  const userId = <?= isset($user['id']) && $user['id'] > 0 ? intval($user['id']) : 'null'; ?>;
</script>
...
<div id="confirmationModal" class="confirmation-modal">
  <div class="confirmation-content">
    ...
    <div class="payment-options">
      ...
      </div><!-- Logos de cartes -->      <button class="confirm-btn">Confirmer le paiement</button>

<div class="card-logos" style="margin-bottom: 15px;">
  <img src="https://img.icons8.com/color/48/000000/visa.png" alt="Visa" style="height: 40px; margin-right: 10px;">
  <img src="https://img.icons8.com/color/48/000000/mastercard-logo.png" alt="Mastercard" style="height: 40px; margin-right: 10px;">
  <img src="https://img.icons8.com/color/48/000000/amex.png" alt="American Express" style="height: 40px;">
</div>

<!-- Formulaire de paiement en ligne -->
<div id="onlinePaymentForm" style="display:none; margin-top: 20px;">
  <!-- Logos de cartes -->
  <div class="card-logos" style="margin-bottom: 15px;">
    <img src="https://img.icons8.com/color/48/000000/visa.png" alt="Visa" style="height: 40px; margin-right: 10px;">
    <img src="https://img.icons8.com/color/48/000000/mastercard-logo.png" alt="Mastercard" style="height: 40px; margin-right: 10px;">
...
<form id="paymentForm" method="POST" action="traitement_paiement.php">
    <label for="cardName">Nom sur la carte</label>
    <input type="text" id="cardName" name="cardName" required style="width: 100%; padding: 10px; margin-bottom: 10px;">

    <label for="cardNumber">Numéro de carte</label>
    <input type="text" id="cardNumber" name="cardNumber" maxlength="19" placeholder="0000 0000 0000 0000" required style="width: 100%; padding: 10px; margin-bottom: 10px;">

    <label for="expiry">Date d'expiration</label>
    <input type="text" id="expiry" name="expiry" placeholder="MM/AA" required style="width: 100%; padding: 10px; margin-bottom: 10px;">

    <label for="cvv">CVV</label>
    <input type="text" id="cvv" name="cvv" maxlength="4" required style="width: 100%; padding: 10px; margin-bottom: 10px;">

    <button type="submit" class="confirm-btn" style="margin-top: 10px;">Payer maintenant</button>
  </form>
</div>


      <button id="cancelOrderBtn" style="margin-top: 10px;background-color: var(--dark-wood);
        color: white;
        border: none;
        padding: 15px 30px;
        font-size: 16px;
        ...
        text-transform: uppercase;
        letter-spacing: 1px;
        margin-top: 20px;">Annuler la commande</button>

    </div>
  </div>
</div>
<div id="messageModal" class="confirmation-modal">
  <div class="confirmation-content">
...
function showConfirmationModal(orderLines) {
    ...
    document.querySelectorAll('.payment-method').forEach(method => {
        method.addEventListener('click', () => {
            ...
            selectedPaymentMethod = method.dataset.method;  // Sauvegarder la méthode sélectionnée
        });
    });

    // Fermeture de la modale
    document.querySelector('.close-confirmation').onclick = () => {
    ...
    document.querySelector('.confirm-btn').onclick = () => {
        ...
        saveOrderToDatabase(orderLines, selectedPaymentMethod);  // Enregistrer la commande avec la méthode de paiement
        modal.classList.remove('active');
    };
}

function saveOrderToDatabase(orderLines, paymentMethod) {
    const userId = 1; // Cela doit être dynamique en fonction de l'utilisateur connecté
    const orderData = {
        user_id: userId,
...
showMessageModal("Commande enregistrée avec echec!");
        }
    })
    .catch(error => {
...
document.getElementById('cancelOrderBtn').addEventListener('click', function () {
    // Réinitialiser les lignes de commande
    orderLines = [];
    ...
    // Optionnel : message à l'utilisateur
    const orderMessage = document.getElementById('orderMessage');
    if (orderMessage) {
        orderMessage.textContent = 'Commande annulée.';
    }
});
function showMessageModal(message) {
  ...
  // Afficher le formulaire de paiement si paiement en ligne est cliqué
  document.querySelectorAll('.payment-method').forEach(method => {
  ...
  document.getElementById("paymentForm").addEventListener("submit", function (e) {
    ...
    // Ici tu peux ajouter une vraie logique de paiement (Stripe, PayPal, etc.)
    alert("Paiement en ligne effectué avec succès !");
    document.getElementById("confirmationModal").style.display = "none";
    document.getElementById("messageModal").style.display = "block";
    document.getElementById("messageContent").textContent = "Votre paiement en ligne a été validé.";
  });

  // Fermer la modale message
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const messageContent = document.getElementById('messageContent');
  const closeMessageBtn = messageModal.querySelector('.close-message');
  ...
  // Confirmer le paiement pour "paiement à la livraison"
  confirmBtn.addEventListener('click', () => {
    ...
    if (selectedPaymentMethod === 'delivery') {
      // Appeler la fonction d'enregistrement de commande avec paiement à la livraison
      saveOrderToDatabase(orderLines, selectedPaymentMethod);
      if (confirmationModal) confirmationModal.classList.remove('active');
    }
  });

  // Soumission du formulaire paiement en ligne
  document.getElementById('paymentForm').addEventListener('submit', e => {
    e.preventDefault();

    if (!selectedPaymentMethod || selectedPaymentMethod !== 'online') {
      alert('Veuillez sélectionner le paiement en ligne.');
      return;
    }
    ...
    .then(response => {
      ...
      // Ici tu peux gérer la réponse du serveur
      showMessageModal('Paiement en ligne effectué avec succès !');
      if (confirmationModal) confirmationModal.classList.remove('active');
      onlineForm.style.display = 'none';
    })
    .catch(err => {
      console.error('Erreur AJAX :', err);
      showMessageModal('Erreur lors du paiement en ligne.');
    });
  });

  // Fonction modale message
  function showMessageModal(message) {
    messageContent.textContent = message;
    messageModal.classList.add('active');
  }

  // Fermeture modale message
